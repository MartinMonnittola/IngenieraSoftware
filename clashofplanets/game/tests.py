...
class GameModelTestsCase(TestCase):
    ...
    """
    def hurt_shield_has_not_value_negative_or_zero(self):
        game = Game.objects.get(pk=1)
        self.assertGreater(game.hurt_to_shield, 0) 

    def hurt_population_not_value_negative_or_zero(self):
        game = Game.objects.get(pk=1)
        self.assertGreater(game.hurt_to_poblation, 0) 

    def const_missil_has_not_value_negative_or_zero(self):
        game = Game.objects.get(pk=1) 
        self.assertGreater(game.const_misil, 0) 

    def const_population_has_not_value_negative_or_zero(self):
        game = Game.objects.get(pk=1) 
        self.assertGreater(game.const_poblation, 0) 

    def const_shield_has_not_value_negative_or_zero(self):
        game = Game.objects.get(pk=1) 
    ...
    def initial_population_has_not_value_negative_or_zero(self): 
        self.assertGreater(game.initial_poblation, 0)

    def can_desactivate_a_planet(self):
        planet = Planet.objects.get(pk=1)
        game = Game.objects.get(pk=1)
        self.assertTrue(game.desactivatePlanet(planet.id))

    def can_not_desactivate_twice_a_planet(self):
        planet = Planet.objects.get(pk=1)
        game = Game.objects.get(pk=1)
    ...
    def can_not_twice_join_game(self):
        game = Game.objects.get(pk=1)
        user = User(username="JJJ", first_name="JJJLJH", last_name= "JJJLKGLVJ",
                    email="dv@gmail.com", password="Aa12345678")
        user.save()
