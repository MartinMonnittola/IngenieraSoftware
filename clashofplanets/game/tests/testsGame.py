...
class GameModelTestsCase(TestCase):
    ...
    def setUp(self):
        user = User(username="GGG", first_name="GGGLJH", last_name= "GGGLKJLVG", email="dvdfv@gmail.com", password="Aa12345678")
        user.save()

        game = Game.create(name="GuerraPlanetaria", max_players=20, owner=user, num_alliances=2, mode=1, bot_def=0, bot_ofc=0)
        game.save()

        team1 = Alliance.create("Team 1", game)
        team1.save()

        planet = Planet.create(player=user, bot=None, game=game, name="Tierra",seed =1, alliance=team1)
        planet.save()
